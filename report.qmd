...
## Model 1.3: Incorporating uncertainty in the likelihood estimation
...
```{r}
#| output: asis
summary(sdr4) %>% as.data.frame() %>% slice(-c(1:348)) %>% 
  xtable %>% print(comment = F,
                   sanitize.text.function = function(x) {x})
```
...
xtable(data.frame(Model = c('AR(1)', 'AR(1) + Pt', 'AR(1) + Pt (ii)', ' $X_t = f(C_t)$'),
                  ...
                  WAIC = c(waic, waic2, waic3, waic4))) %>% print(comment = F,
                                                                  sanitize.text.function = function(x) {x})
```
...
#| results: hide
subject = obj
jit = replicate(100, nlminb(subject$par+rnorm(length(subject$par), sd=.25),subject$fn,subject$gr)$par)
```
